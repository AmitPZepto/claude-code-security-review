"""Claude AI model client implementation."""
...
from ..constants import RATE_LIMIT_BACKOFF_MAX, PROMPT_TOKEN_LIMIT
...
class ClaudeClient(AIModelClient):
    ...
    def call_with_retry(self, 
        ...
        while retries <= self.config.max_retries:
            try:
                logger.info(f"Claude API call attempt {retries + 1}/{self.config.max_retries + 1}")
        ...
        return False, "", f"API call failed after {self.config.max_retries + 1} attempts: {last_error}"
